...
from model.glip_mebow.utils import HiddenPrints
...
class DepthEstimationModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, model_type='DPT_Large'):
        ...
        with HiddenPrints('DepthEstimation'):
            ...
            depth_estimation_model = hub.load('model/glip_mebow/MiDaS/', model_type, pretrained=False, source='local').to('cpu')
            depth_estimation_model.load_state_dict(torch.load('model/glip_mebow/pretrained_models/depth/dpt_large_384.pt', map_location='cpu'))
            depth_estimation_model = depth_estimation_model.to(self.dev)
            depth_estimation_model.eval()

            midas_transforms = torch.hub.load("model/glip_mebow/MiDaS", "transforms", source='local')
...
class GLIPModel(BaseModel):
    ...
    def __init__(self, model_size='large', gpu_number=0, *args):
        ...
        working_dir = f'model/glip_mebow/{config.path_pretrained_models}/GLIP/'
        if model_size == 'tiny':
...
class CodexModel(BaseModel):
    ...
    def forward(self, prompt=None, input_type='image', prompt_file=None, base_prompt=None, extended_prompt=None):
        ...
        if extended_prompt is None and prompt is not None:
